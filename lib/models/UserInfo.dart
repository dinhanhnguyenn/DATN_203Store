...
Future<UserInfo?> getUserInfo(int orderId) async {
  try {
    final response = await http.get(Uri.parse(
        'http://192.168.1.9/flutter/InformationUser.php?order_id=$orderId'));
...
Future<int?> getTotalQuantity(int orderId) async {
  try {
    final response = await http.get(
        Uri.parse('http://192.168.1.9/flutter/quantity.php?order_id=$orderId'));
    if (response.statusCode == 200) {
      final jsonData = json.decode(response.body);
      if (jsonData['success']) {
...
Future<double?> getTotalAmount(int orderId) async {
  try {
    final response = await http.get(Uri.parse(
        'http://192.168.1.9/flutter/TotalOrder.php?order_id=$orderId'));
    if (response.statusCode == 200) {
      final jsonData = json.decode(response.body);
      if (jsonData['success']) {
...
Future<List<Map<String, dynamic>>?> getProducts(int orderId) async {
  try {
    final response = await http
        .get(Uri.parse('192.168.1.9/flutter/NamePrice.php?order_id=$orderId'));
    if (response.statusCode == 200) {
      final jsonData = json.decode(response.body);
      if (jsonData['success']) {
